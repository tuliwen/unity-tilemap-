...
public class NewBehaviourScript : MonoBehaviour {
    ...
    private Tile baseTile;//使用的最基本的Tile，我这里是白色块，然后根据数据设置不同颜色生成不同Tile
    Tile[] arrTiles;//生成的Tile数组
    void Awake()
    {
        //ins = this;
        baseTile = (Tile)Resources.Load("newtilemap/TreeOakB", typeof(Tile));
    ...
    IEnumerator InitData()
    {
       ...
       // for (int i = 0; i < colorCount; i++)
        ...
        {//这里就是设置每个Tile的信息了
            ...
            {
                tilemap.SetTile(new Vector3Int(j, i, 0), arrTiles[0]);
            }
            yield return null;
        }
